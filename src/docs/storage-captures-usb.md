...
After recording stops, the daemon generates `capture_...jpg` beside the capture file using `ffmpeg`. It tries a few seek points and writes through a temporary file before replacing the final JPG. The web UI only surfaces capture cards whose thumbnails are ready, so a fresh recording appears complete.
