...
if tart list | grep -q "$VM_NAME"; then
    ...
    echo "To recreate: tart delete $VM_NAME && ./src/buildroot/scripts/vm-setup.sh"
    exit 0
fi
...
echo "    IP: $VM_IP"
echo "    SSH: ssh -i $SSH_KEY admin@$VM_IP   (or ./src/buildroot/scripts/vm-ssh.sh)"
echo ""
echo "==> Stopping VM. Use './src/buildroot/scripts/build.sh' to build images."
